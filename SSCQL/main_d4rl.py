...
from utils.evaluation import evaluate_d4rl, flatten
...
flags.DEFINE_string('env_name', 'halfcheetah-medium-v2', 'Environment (dataset) name.')
...
config_flags.DEFINE_config_file('agent', 'agents/ssc_flow_policy.py', lock_config=False)


flags.DEFINE_string('wandb_project', 'OSC_D4RL_Tasks_', 'WandB project name.')


def main(_):
    # Set up logger.
    # exp_name = get_exp_name(FLAGS.seed)
    exp_name = f"{FLAGS.env_name}"
    ...
    for i in tqdm.tqdm(range(1, FLAGS.offline_steps + FLAGS.online_steps + 1), smoothing=0.1, dynamic_ncols=True):
        ...
        if FLAGS.eval_interval != 0 and (i == 1 or i % FLAGS.eval_interval == 0):
            renders = []
            eval_metrics = {}
            eval_info, trajs, cur_renders = evaluate_d4rl(
                agent=agent,
                env=eval_env,
                config=config,
            ...
            print(f"Episode normalized return: {eval_info['episode.normalized_return']}, std: {eval_info['episode.std_normalized_return']}")
