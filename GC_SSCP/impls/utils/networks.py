...
class ShortcutActorVectorField(nn.Module):
    ...
    def setup(self) -> None:
        self.mlp = MLP((*self.hidden_dims, self.action_dim+self.goal_rep_dim),
                        activate_final=False, layer_norm=self.layer_norm, activations=mish)
        self.time_encoder = FourierFeatures(self.time_dim)
        self.time_feature_encoder = MLP((self.time_dim,)*2, activate_final=False, 
                                        layer_norm=self.layer_norm, activations=mish)
        self.step_feature_encoder = MLP((self.time_dim,)*2, activate_final=False,
                                        layer_norm=self.layer_norm, activations=mish)

    @nn.compact
    def __call__(self, observations, goals, actions, subgoal_reps, times=None, steps=None, is_encoded=False):
        ...
        """
        if self.encoder is not None:
            inputs = [self.encoder(observations, goals, goal_encoded=is_encoded), actions, subgoal_reps]
        else:
            inputs = [observations, goals, actions, subgoal_reps]
        if times is not None:
            encoded_times = self.time_encoder(times)
            encoded_times = self.time_feature_encoder(encoded_times)
            if steps is not None:
                encoded_steps = self.time_encoder(steps)
                encoded_steps = self.step_feature_encoder(encoded_steps)
                encoded_times += encoded_steps
            inputs.append(encoded_times)
        inputs = jnp.concatenate(inputs, axis=-1)
            

        v = self.mlp(inputs)
        action_v = v[..., :self.action_dim]
        subgoal_v = v[..., self.action_dim:]

        return action_v, subgoal_v
